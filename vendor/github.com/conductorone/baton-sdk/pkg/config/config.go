...
func DefineLambdaConfiguration(
	ctx context.Context,
	...
	confschema.Fields = append(confschema.Fields, field.LambdaFields()...)
	...
	defaultFields := append(field.DefaultFields, field.LambdaFields()...)
...
func DefineConfiguration(
	...
	lambdaServerCmd := &cobra.Command{
		Use:    "_lambda-connector-service",
		Short:  "Start the connector service lambda handler",
		Hidden: true,
		RunE:   cli.MakeLambdaServerCommand(ctx, connectorName, v, confschema, connector),
	}
	err = setFlagsAndConstraints(lambdaServerCmd, schema)
	if err != nil {
		return nil, nil, err
	}
	mainCMD.AddCommand(lambdaServerCmd)
